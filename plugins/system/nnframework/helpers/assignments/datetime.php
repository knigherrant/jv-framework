...
class NNFrameworkAssignmentsDateTime extends NNFrameworkAssignment
{
	...
	function passTime()
	{
		...
		$up = strtotime(JFactory::getDate($this->date->format('Y-m-d ', true) . $this->params->publish_up)->setTimeZone($tz));
		$down = strtotime(JFactory::getDate($this->date->format('Y-m-d ', true) . $this->params->publish_down)->setTimeZone($tz));
