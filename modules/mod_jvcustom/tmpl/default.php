...
<div 
	...
	style="<?php echo (!empty($background))?'background-image:url('.JUri::root(true).'/'.$background.');':''; ?> <?php echo (!empty($position))?'background-position: '.$position.';':''; ?> <?php echo (!empty($size))?'background-size: '.$size.';':''; ?> <?php echo (!empty($backgroundColor))?'background-color: '.$backgroundColor.';':''; ?><?php echo (!empty($parallax))?' background-attachment: fixed;':''; ?>">
	<?php if (!empty($video)) {?>
	<div class="player"  
		data-property="{
