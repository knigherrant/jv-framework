...
$document->addStyleSheet('components/com_jvss/assets/css/jvss.css');

$user	= JFactory::getUser();
$userId	= $user->get('id');
$listOrder	= $this->state->get('list.ordering');
$listDirn	= $this->state->get('list.direction');
$canOrder	= $user->authorise('core.edit.state', 'com_jvss');
$saveOrder	= $listOrder == 'a.ordering';
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_jvss&task=items.saveOrderAjax&tmpl=component';
...
<form action="<?php echo JRoute::_('index.php?option=com_jvss&view=items'); ?>" method="post" name="adminForm" id="adminForm">
...
<?php endif;?>
		...
		<table class="table table-striped" id="itemList">
			<thead>
				...
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_JVSS_ITEMS_NAME', 'a.name', $listDirn, $listOrder); ?>
				</th>
                    
                    
                <?php if (isset($this->items[0]->id)): ?>
                    <th width="1%" class="nowrap center">
                        <?php echo JHtml::_('grid.sort', 'COM_JVSS_ITEMS_SHORTCODE', 'a.id', $listDirn, $listOrder); ?>
                    </th>
                <?php endif; ?>
                <?php if (isset($this->items[0]->id)): ?>
                    <th width="1%" class="nowrap center">
			...
			<?php foreach ($this->items as $i => $item) :
                ...
                $canCreate	= $user->authorise('core.create',		'com_jvss');
                $canEdit	= $user->authorise('core.edit',			'com_jvss');
                $canCheckin	= $user->authorise('core.manage',		'com_jvss');
                $canChange	= $user->authorise('core.edit.state',	'com_jvss');
				?>
				...
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_jvss&task=item.edit&id='.(int) $item->id); ?>">
                ...
                <?php if (isset($this->items[0]->id)): ?>
                    <td class="center hidden-phone">
                        <code>{loadjvss <?php echo (int) $item->id; ?>}</code>
                    </td>
                <?php endif; ?>
                ...
                <?php if (isset($this->items[0]->id)): ?>
                    <td class="center">
                        <a
                        href="<?php echo JUri::base( true ) . "/index.php?option=com_jvss&task=item.sexport&id={$item->id}"; ?>"
