...
function apply_img(file,type_file,external){
    ...
    if (external=="") {
	    ...
	    var target = window_parent.document.getElementById(external);
        target = window_parent.jQuery( target );
	    target.val( $.trim( base_url, '/' ) + path + file ).change();
        if( track )
        {
            if( track.match( /^item-mce/ ) )
            {
                var mce = window_parent.tinymce.get( track );
                mce.windowManager.close();
                return false;
            }else if( track.match( /imageok/ ) )
            {
                window_parent.jQuery( '#' + track ).modal( 'hide' );
                return false;
            }
        }
	    close_window();
...
function apply_video(file,type_file,external){
	...
	$(closed).find('.mce-close').trigger('click');
        ...
        if( window_parent.tinymce && window_parent.tinymce.get( track ) )
        {
            target.value = base_url+path+file;
            
            window_parent.tinymce.get( track ).windowManager.close();
        }
        else {             
            $(target).val(base_url+path+file);    
            close_window();
