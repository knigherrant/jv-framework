<div class="featured-box featured-box-cart">
	<div class="box-content">
		...
		<table class="table shop_table">
			<thead>
				<tr>
					<th><?php echo vmText::_ ('TPL_CART_IMAGE') ?></th>
					<th style="min-width: 100px;"><?php echo vmText::_ ('COM_VIRTUEMART_CART_NAME') ?></th>
					<th
			...
			<tbody>
				...
				<tr class="sectiontableentry<?php echo $i ?>">
					...
					<td >
						...
						<small><?php echo vmText::_ ('COM_VIRTUEMART_CART_SKU') ?>: <?php  echo $prow->product_sku ?></small>
					</td>
					<td>
						<?php
					...
					<td ><?php
							...
							<div class="input-group">
						      <input type="text"
								   ...
								   title="<?php echo  vmText::_('COM_VIRTUEMART_CART_UPDATE') ?>" class=" js-recalculate form-control" size="3" maxlength="4" name="quantity[<?php echo $pkey; ?>]" value="<?php echo $prow->quantity ?>" />
						      <div class="input-group-btn">
						        <button type="submit" class="vmicon vm2-add_quantity_cart btn" name="updatecart.<?php echo $pkey ?>" title="<?php echo  vmText::_ ('COM_VIRTUEMART_CART_UPDATE') ?>" ><i class="fa fa-undo"></i></button>
								<button type="submit" class="vmicon vm2-remove_from_cart btn" name="delete.<?php echo $pkey ?>" title="<?php echo vmText::_ ('COM_VIRTUEMART_CART_DELETE') ?>" ><i class="fa fa-times"></i></button>
						      </div>
						    </div><!-- /input-group -->
					</td>

					<?php if (VmConfig::get ('show_tax')) { ?>
					<td><?php echo "<span class='priceColor2'>" . $this->currencyDisplay->createPriceDiv ('taxAmount', '', $prow->prices, FALSE, FALSE, $prow->quantity, false, true) . "</span>" ?></td>
					<?php } ?>
					...
					<td class="sub-toltal">
						...
						if (VmConfig::get ('checkout_show_origprice', 1) && !empty($prow->prices['basePriceWithTax']) && $prow->prices['basePriceWithTax'] != $prow->prices['salesPrice']) {
							echo '<div class="line-through">' . $this->currencyDisplay->createPriceDiv ('basePriceWithTax', '', $prow->prices, TRUE, FALSE, $prow->quantity) . '</div><br />';
						}
						elseif (VmConfig::get ('checkout_show_origprice', 1) && empty($prow->prices['basePriceWithTax']) && $prow->prices['basePriceVariant'] != $prow->prices['salesPrice']) {
							echo '<div class="line-through">' . $this->currencyDisplay->createPriceDiv ('basePriceVariant', '', $prow->prices, TRUE, FALSE, $prow->quantity) . '</div><br />';
						}
				...
				<!-- footer -->
					...
					<tr class="sectiontableentry1">
						<td colspan="4" ><h4 class="mb-0"><?php echo vmText::_ ('COM_VIRTUEMART_ORDER_PRINT_PRODUCT_PRICES_TOTAL'); ?></h4></td>
						...
						<td class="sub-toltal"><?php echo $this->currencyDisplay->createPriceDiv ('salesPrice', '', $this->cart->cartPrices, FALSE) ?></td>
					</tr>
					...
					<tr class="sectiontableentry2 total sectionDark">
						...
						<td  class="text-bold sub-toltal"><?php echo $this->currencyDisplay->createPriceDiv ('billTotal', '', $this->cart->cartPrices['billTotal'], FALSE); ?></td>
					</tr>
					<?php
