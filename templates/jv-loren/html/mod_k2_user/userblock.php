...
<div id="k2ModuleBox<?php echo $module->id; ?>" class="k2UserBlock<?php if($params->get('moduleclass_sfx')) echo ' '.$params->get('moduleclass_sfx'); ?>">
  ...
  <?php endif; ?>
	...
	<div class="k2UserBlockDetails clearfix">
	  ...
	  <?php endif; ?>
	  <h5 class="ubName"><?php echo $user->name; ?></h5>
	  <p class="ubCommentsCount"><?php echo JText::_('K2_YOU_HAVE'); ?> <b><?php echo $user->numOfComments; ?></b> <?php if($user->numOfComments==1) echo JText::_('K2_PUBLISHED_COMMENT'); else echo JText::_('K2_PUBLISHED_COMMENTS'); ?></p>
	</div>

  <ul class="k2UserBlockActions">
		<?php if(is_object($user->profile) && isset($user->profile->addLink)): ?>
		<li>
			<a class="edit-modal" rel="{handler:'iframe',size:{x:990,y:550}}" href="<?php echo $user->profile->addLink; ?>"><?php echo JText::_('K2_ADD_NEW_ITEM'); ?></a>
		</li>
		<?php endif; ?>
		<li>
		...
		<li>
			<a class="edit-modal" rel="{handler:'iframe',size:{x:990,y:550}}" href="<?php echo JRoute::_('index.php?option=com_k2&view=comments&tmpl=component'); ?>"><?php echo JText::_('K2_MODERATE_COMMENTS_TO_MY_PUBLISHED_ITEMS'); ?></a>
		</li>
		<?php endif; ?>
	</ul>
	<?php if($menu): ?>
  ...
  <form action="<?php echo JURI::root(true); ?>/index.php" method="post">
    <button type="submit" name="Submit" class="btn btn-primary btn-sm ubLogout" ><?php echo JText::_('K2_LOGOUT'); ?></button>
