...
?>
<?php if($items):?>
<div id="<?php echo "mod-frm-portfolio-{$module->id}"?>" class="pfo-module portfolio-style3 portfolio<?php echo number_format(12/$column, 0)?> <?php echo implode(' ', array($params->get('moduleclass_sfx', '')))?>">
   ...
   <div class="clearfix topPortfolio">
    ...
    <div class="box-portfolio <?php echo($isFilter ? 'portfolioContainer': '')?> ">
        ...
        <?php foreach($items as $item):?>
		 <div id="pfo-item-<?php echo $item->id?>" class="pfo-item col-xxs-12 col-xs-6 col-sm-<?php echo ($params->get('limit')%2 == 0)?'6':'4'; ?> col-md-<?php echo $column?>" data-groups='[<?php echo $item->aliasTags?>]' data-name="<?php echo strtolower($item->name)?>" data-date="<?php echo strtotime($item->date_created)?>" data-like="<?php echo $item->cliked?>">
          <div class="pfo-body">
             <div class="pfo-image" style="background-image: url(<?php echo $item->image ?>)">
             <img class="hidden" src="<?php echo $item->image ?>" alt="<?php echo $item->name?>">
            </div>
            <!-- end img  -->
    ...
    <?php if($total):?>
    <?php require ModJvPortfolioHelper::loadTemplate('Style3_nav')?>
    <?php endif;?>
</div>
<?php endif;?>