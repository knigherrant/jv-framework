...
?>
<?php if($items):?>
<div id="<?php echo "mod-frm-portfolio-{$module->id}"?>" class="portfolio-style13 portfolio<?php echo number_format(12/$column, 0)?> <?php echo implode(' ', array($params->get('moduleclass_sfx', '')))?>">
   ...
   <div class="clearfix topPortfolio">
    ...
    <div class="box-portfolio <?php echo($isFilter ? 'portfolioContainer': '')?> ">
        ...
        <?php foreach($items as $item):?>
		 <div id="pfo-item-<?php echo $module->id.'-'.$item->id?>" class="pfo-item col-xs-6 col-sm-<?php echo ($column !=6)?'4':'6'; ?> col-md-<?php echo $column?>" data-groups='[<?php echo $item->aliasTags?>]' data-name="<?php echo strtolower($item->name)?>" data-date="<?php echo strtotime($item->date_created)?>" data-like="<?php echo $item->cliked?>">
          <div class="pfo-body">
            <div class="pfo-image">
            ...
            <div class="pfo-content">            
              ...
              <div class="pfo-tool">
                ...
                <?php if($params->get('showLiked', 0)):?>
                 <a class="pfo-like pull-right ml-10" href="<?php echo JUri::root()."?option=com_jvportfolio&amp;task=items.toggleVote&amp;pfid={$item->id}"?>" data-pfvote="<?php echo $item->id?>"><i class="<?php echo ($item->lactive ? 'active' : '')?> fa fa-heart-o">&nbsp;<?php echo $item->cliked?></i></a> 
                 <?php endif;?>
    ...
    <?php if($total):?>
    <?php require ModJvPortfolioHelper::loadTemplate('Style13_nav'); ?>
    <?php endif;?>
</div>
<?php endif;?>