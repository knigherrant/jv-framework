...
?>
<?php if($items):?>
<div id="<?php echo "mod-frm-portfolio-{$module->id}"?>" class="portfolio-style18 portfolio<?php echo number_format(12/$column, 0)?> <?php echo implode(' ', array($params->get('moduleclass_sfx', '')))?>">
   ...
   <div class="clearfix topPortfolio">
              ...
              <div class="pfo-content-table">
                <div class="pfo-content-table-cell">
                  ...
                  <div class="pfo-overlay">
                       <?php if($item->gallery):?> 
                       <a class="link-quick btn btn-sm btn-white" href="#" data-imgs='<?php echo json_encode($item->gallery)?>' data-qview="lightbox" title="<?php echo $item->name?>"><?php echo JText::_('TPL_PORTFOLIO_ZOOM')?></a>
                       <?php endif?>
                       <a class="link-detail btn btn-sm btn-white" href="<?php echo JRoute::_("index.php?option=com_jvportfolio&view=item&id={$item->id}")?>" title="<?php echo $item->name?>"><?php echo JText::_('TPL_PORTFOLIO_VIEW')?></a>               
                  </div>
                </div>
              </div>
            </div>
    ...
    <?php if($total):?>
    <?php require ModJvPortfolioHelper::loadTemplate('Style17_nav');?>
    <?php endif;?>
</div>
<?php endif;?>