...
?>
<?php if($items):?>
<div id="<?php echo "mod-frm-portfolio-{$module->id}"?>" class="portfolio-style8 portfolio<?php echo number_format(12/$column, 0)?> <?php echo implode(' ', array($params->get('moduleclass_sfx', '')))?>">
   ...
   <div class="clearfix topPortfolio">
    ...
    <div class="box-portfolio <?php echo($isFilter ? 'portfolioContainer': '')?> ">
     ...
     <div id="pfo-item-<?php echo $item->id?>" class="pfo-item col-xs-6 col-sm-<?php echo (number_format(12/$column, 0) == 3)?'4':'6'; ?> col-md-<?php echo $column?>" data-groups='[<?php echo $item->aliasTags?>]' data-name="<?php echo strtolower($item->name)?>" data-date="<?php echo strtotime($item->date_created)?>" data-like="<?php echo $item->cliked?>">
          <div class="pfo-body">
            <div class="pfo-image">
              <div class="pfo-overlay">
                   <?php if($item->gallery):?> 
                           <a class="link-detail" href="#" data-imgs='<?php echo json_encode($item->gallery)?>' data-qview="lightbox" title="<?php echo $item->name?>"><i class="fa fa-search"></i></a>
                           <?php endif?>
              </div>
    ...
    <?php if($total):?>
    <?php require ModJvPortfolioHelper::loadTemplate('Style8_nav'); ?>
    <?php endif;?>
</div>
<?php endif;?>