...
if ($this->allowRating || $this->allowReview || $this->showRating || $this->showReview) {
	...
	for( $num = 0; $num<=$maxrating; $num++ ) {
		...
		$stars2[] = '
				<div title="'.(vmText::_( "COM_VIRTUEMART_RATING_TITLE" ).$num.'/'.$maxrating).'" class="ratingbox" style="margin: 0; display:inline-block;width:'. 16*$maxrating.'px;">
					<div style="width:'.(16*$num).'px">
					</div>
				</div>';
	}
...
if ($this->showReview) {
	...
	?>
	<h6 class="mt-0 mb-30 text-uppercase text-semi-bold"><?php echo $reviews. ' '  . JText::_("TPL_REVIEW_FOR") . ' "' . $this->product->product_name ?>"</h6>
					...
					if (!empty($this->rating_reviews) && $review->published) {
						...
						<li>
							<div class="comment">
								...
								?>
								<div class="comment-avatar"> <img src="<?php echo $grav_url; ?>" alt="<?php echo $review->name;?>" width="80" onError="this.src='<?php echo JUri::base() ?>templates/jv-loren/images/avatar/default.jpg';"  /> </div>
...
if ($this->allowRating or $this->allowReview) {
	...
	if ($review_editable) {
		...
		<div class="post-block post-leave-comment">
			<form method="post"
				  ...
				  name="reviewForm" id="reviewform" class="form-5">

				<?php if($this->allowRating and $review_editable) { ?>

					<h6 class="mt-0 mb-30 text-uppercase text-semi-bold"><?php echo vmText::_( 'COM_VIRTUEMART_WRITE_REVIEW' );
				...
				if($review_editable and $this->allowReview) { ?>
					<div class="row">
						<div class="col-md-12">
							<input class="btn btn-default btn-sm btn-outline" type="submit" onclick="return( check_reviewform());"
				...
				<?php } else if($review_editable and $this->allowRating) { ?>
					<div class="row">
						<div class="col-md-12">
							<input class="btn btn-default btn-sm btn-outline" type="submit" name="submit_review"
	...
	<?php
		...
		echo '<h6 class="mt-0 mb-30 text-uppercase text-semi-bold">'.vmText::_( 'COM_VIRTUEMART_DEAR' ).$this->user->name.'</h6>';
