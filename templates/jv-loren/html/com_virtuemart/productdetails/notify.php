...
<form method="post" action="<?php echo JRoute::_('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id='.$this->product->virtuemart_product_id.'&virtuemart_category_id='.$this->product->virtuemart_category_id, FALSE) ; ?>" name="notifyform" id="notifyform">
	...
	<div>
		<input type="text" class="form-control" name="notify_email" value="<?php echo $this->user->email; ?>" />
		<div class="addtocart-button pt-10"><input type="submit" name="notifycustomer"  class="btn btn-primary notify-button" value="<?php echo vmText::_('COM_VIRTUEMART_CART_NOTIFY') ?>" title="<?php echo vmText::_('COM_VIRTUEMART_CART_NOTIFY') ?>" /></div>
	</div>
