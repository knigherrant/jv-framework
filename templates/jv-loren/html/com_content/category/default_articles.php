...
<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm" class="form-inline">
	...
	<fieldset class="filters">
		...
		<div class="filter-search inline-block">
			<label class="filter-search-lbl" for="filter-search"><?php echo JText::_('COM_CONTENT_'.$this->params->get('filter_field').'_FILTER_LABEL').'&#160;'; ?></label>
			<input type="text" name="filter-search" id="filter-search" value="<?php echo $this->escape($this->state->get('list.filter')); ?>" class="form-control" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_CONTENT_FILTER_SEARCH_DESC'); ?>" />
		</div>
	<?php endif; ?>

	<?php if ($this->params->get('show_pagination_limit')) : ?>
		<div class="display-limit inline-block" >
	...
	<table class="category table table-striped table-bordered table-hover">
		...
		<thead>
			<tr>
				...
				<th class="list-hits text-center" id="tableOrdering4">
		...
		<tbody>
			...
			<tr class="cat-list-row<?php echo $i % 2; ?>">
				...
				<?php if (in_array($article->access, $this->user->getAuthorisedViewLevels())) : ?>
					...
					<?php if ($this->params->get('list_show_hits', 1)) : ?>
					<td class="list-hits text-center">
