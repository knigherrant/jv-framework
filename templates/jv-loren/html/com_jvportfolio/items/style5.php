...
?>
<?php if($this->items):?>
<div id="frm-portfolio" class="portfolio-style5 portfolio<?php echo number_format(12/$column, 0)?>">
    ...
    <div class="box-portfolio row <?php echo($isFilter ? 'portfolioContainer': '')?>">  
        ...
        <div id="pfo-item-<?php echo $item->id?>" class="pfo-item col-xxs-12 col-xs-6 col-sm-<?php echo ($column !=6)?'4':'6'; ?> col-md-<?php echo $column?>" data-groups='[<?php echo $item->aliasTags?>]' data-name="<?php echo strtolower($item->name)?>" data-date="<?php echo strtotime($item->date_created)?>" data-like="<?php echo $item->cliked?>">
          <div class="pfo-body">
            ...
            <!-- end img  -->
            <div class="pfo-overlay clearfix">
                 <?php if($item->gallery):?> 
                 <!-- <a class="link-quick" href="#" data-imgs='<?php echo json_encode($item->gallery)?>' data-qview="lightbox"><i class="fa fa-search"></i></a> -->
                 <?php endif?>
                 <!-- <a class="link-detail" href="<?php echo JRoute::_("index.php?option=com_jvportfolio&view=item&id={$item->id}")?>"><i class="fa fa-plus"></i></a>                -->
            </div>
